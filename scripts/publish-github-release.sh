...
shopt -s nullglob
FILES=("$ARTIFACT_DIR"/2fado-*-linux-* "$ARTIFACT_DIR"/2fado-*-darwin-* "$ARTIFACT_DIR"/2fado-*-windows-*)
...
if [ ${#FILES[@]} -eq 0 ]; then
	echo "::error::No release artifacts (2fado-*-<os>-<arch>[.exe], SHA256SUMS) found in $ARTIFACT_DIR." >&2
	exit 1
fi
