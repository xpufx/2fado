...
- **`user-attention`**: Escalation signal for blocked or ambiguous issues. Used when an issue is sitting and the required deliverable (or next action) is fundamentally unclear.
  ...
  - **Mandatory Requirement**: Whenever applying `user-attention`, the agent **MUST** post a clear, precise comment directly addressing the operator (`@operator`) stating what options exist and what exact clarification or decision is required to unblock execution.
...
1. **Deterministic Baseline (`forgejo-issues-check`)**:
   - Run `forgejo-issues-check` to get the ranked list of unblocked, prioritized candidates.
...
5. Stand by for fast review from the `Orchestrator` or testing by human operator.
