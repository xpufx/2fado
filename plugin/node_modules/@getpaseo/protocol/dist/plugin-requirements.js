...
export function validatePluginRequirements(requirements) {
    const range = requirements?.paseo;
    if (range !== undefined && (!range.trim() || validRange(range) === null)) {
        throw new Error(`Invalid requirements.paseo: ${JSON.stringify(range)}. Use an npm semver range such as ">=0.8.0".`);
    }
}
export function assertPluginCompatibility(input) {
    ...
    if (!version) {
        throw new Error(`Cannot check plugin "${input.id}" requirements: Paseo ${input.runtime} version is unknown. Update the ${input.runtime}.`);
    }
    const stableCore = `${version.major}.${version.minor}.${version.patch}`;
    if (satisfies(version, range) || satisfies(stableCore, range))
        return;
    const action = input.requirements?.paseo === undefined
    ...
    throw new Error(`Plugin "${input.id}" requires Paseo ${range}. Your ${input.runtime} is ${input.version}. ${action}`);
}
//# sourceMappingURL=plugin-requirements.js.map