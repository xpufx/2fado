...
export class DaemonClient {
    constructor(config) {
        this.config = config;
        this.providerSnapshotUpdates = new ProviderSnapshotUpdates({
        ...
        this.subscriptions = new ConnectionSubscriptions({
            ...
            failed: (error) => this.logger.error({ err: error }, "Failed to update connection subscriptions"),
        });
        this.transport = null;
    ...
    async markWorkspaceUnread(workspaceId, requestId) {
        const response = await this.sendNamespacedCorrelatedSessionRequest({
            requestId,
            message: {
    ...
    async runWorkspaceSetup(workspaceId, requestId) {
        return this.sendNamespacedCorrelatedSessionRequest({
            requestId,
    ...
    async createAgent(options) {
        if (options.idempotencyKey !== undefined)
            this.requireAgentRequestReceipts();
    ...
    async appendAgentTimelineItem(agentId, item) {
        const requestId = this.createRequestId();
        const payload = await this.sendCorrelatedSessionRequest({
            requestId,
    ...
    subscribeAgentTimeline(agentId, handler) {
    ...
    updateEventSubscriptions() {
        ...
        this.subscriptions.setEvents(events.filter((type) => this.eventListeners.size > 0 || (this.messageHandlers.get(type)?.size ?? 0) > 0));
    }
    async sendEventSubscription(events) {
        if (this.connectionState.status !== "connected")
            return;
        // COMPAT(explicitEventSubscriptions): added in v0.8.0, remove legacy broadcast handling after 2027-03-08.
        if (!this.lastServerInfoMessage?.features?.explicitEventSubscriptions ||
    ...
    async sendTimelineSubscription(agentIds) {
        if (this.connectionState.status !== "connected")
            return;
    ...
    requestProvidersSnapshot(options) {
        return this.sendCorrelatedSessionRequest({
            requestId: options?.requestId,
            message: {
    ...
    handleTransportMessage(data) {
        ...
        const isOpen = this.beginTraceSection("paseo.ws.frame.inbound", describeInboundTransportFrame(rawData, rawBytes));
        try {
    ...
    deliverSessionMessage(msg) {
        const consumerMessage = normalizeProviderSnapshotUpdateMessage(msg, this.config.providerSnapshots !== "wire");
