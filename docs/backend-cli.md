...
2fado daemon | 2fado run -- <argv...> | 2fado approve|deny <id>
  ...
  | 2fado ack <id> | 2fado status <id> | 2fado version | 2fado socket-version
```
...
Note: the plugin's `socketCandidates()` chain is per-call `input.socketPath` → `$TWOFADO_SOCKET` → `$FADO_SOCKET` → `/tmp/2fado.sock`. `docs/paseo-plugin.md` also lists `/run/2fado.sock`; code does not — drift to fix when freezing the contract.
...
Each subcommand is one JSON-lines envelope (`protocol.ClientMessage`, exactly one field set) over the unix socket, one JSON line back, fresh connection per call: `run`, `verdict`, `notify`, `ack`, `status`, `version`. Consumer-side plugin ops (`list`, `recent`, `telegram_info`, `telegram_set_config`, `policy_add_rule`) have no CLI subcommand today.
