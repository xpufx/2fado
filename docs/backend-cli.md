...
Status: draft. Daemon-side refs: `go/cmd/2fado/main.go` (subcommands, usage, env), `go/internal/client/client.go` (socket ops, exit codes), `go/internal/protocol/protocol.go` (envelope, request shapes — source of truth, `ClientMessage` 13 ops).
...
- `ask --question <q> --option <o> [--option <o>...] [--link <url>] [--ttl <dur>]` — create an interactive multi-choice question petition (operator picks one of the options). Sends `{ask:{question,options,link?,ttl_seconds?}}` (requires at least two options). Blocks until operator selects or TTL expires. Prints chosen option to stdout and exits 0 on selection, 2 on timeout, 1 on rejection/transport error.
...
Each subcommand is one JSON-lines envelope over the unix socket, one JSON line back, fresh connection per call: run, verdict, notify, ask, ack, status, version, list, recent (9 socket ops). Daemon-only ops (telegram_info, telegram_set_config, policy_add_rule, help) have no CLI subcommand (help is available via direct socket query).
